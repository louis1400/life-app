...
function actionLabel(e:GroceryEvent) {
  switch(e.action.kind) {
    ...
    case "queue":return `Refill list: ${e.action.count} packs`;
...
export default function Groceries() {
  ...
  const started=PRODUCTS.filter(p=>states[p.id].lastUsageOn!==null).length;
  ...
  return <main className="workspace">
    ...
    <header className="masthead"><a className="wordmark" href="/" aria-label="life-app home"><span className="brand-mark">l</span>life-app</a><span className="header-divider"/><span className="section-label">Groceries</span><span className="private-label">Your personal space</span></header>
    <div className="page-heading"><div><p className="eyebrow">A lighter walk home</p><h1>Your everyday essentials.</h1><p className="intro">Keep track of what you have. Find your refill rhythm.</p></div><div className="learning-pill"><Clock3 size={16}/>{observed===13?"Estimates ready":"Learning your routine"}</div></div>
    ...
    <section className="learning-banner" aria-label="Tracking progress"><div className="banner-copy"><span className="banner-icon"><Package size={24}/></span><div><h2>{started===0?"No guessing needed.":"A little tracking. A lighter load."}</h2><p>{started===0?"When you open a fresh pack, start tracking it. We'll learn from when it runs out.":"Log a finished pack when you notice it. Refill estimates improve as you go."}</p></div></div><div className="banner-stats"><div><strong>13</strong><span>chosen essentials</span></div><div><strong>{loaded?observed:"—"}<small> / 13</small></strong><span>with a usage estimate</span></div></div></section>
    <div className="content-grid"><section className="essentials" aria-label="Your selected products"><div className="list-heading"><h2>At home</h2><span>{loading?"Loading your stock…":"Track one whole pack at a time"}</span></div>
      {Array.from(new Set(PRODUCTS.map(p=>p.category))).map(category=><section className="product-group" key={category}><h3 className="category-label">{category}</h3><div className="product-list">{PRODUCTS.filter(p=>p.category===category).map(p=>{
        ...
        return <article className="product-row" key={p.id}>
          ...
          <div className="product-actions"><Button className="track-button" variant={s.inUse?"default":"outline"} disabled={disabled} onClick={()=>p.mode==="usage"?void update(p,{kind:"used"}):openProduct(p)}>{p.mode==="usage"?"Used one":s.inUse?"Log finish":"Start tracking"}</Button><Button variant="ghost" size="icon" className="queue-button" disabled={disabled||s.queuePacks>=999} onClick={()=>void update(p,{kind:"queue",count:s.queuePacks+1})} aria-label={`Add one pack of ${p.shortName} to refill list`} title="Add to refill list"><Plus size={18}/></Button></div>
        </article>;
      })}</div></section>)}

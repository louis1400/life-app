...
export async function GET(request:Request,context:{params:Promise<{id:string}>}){try{
 const user=owner(request),{id}=await context.params;const row=await db().prepare('SELECT * FROM archive_items WHERE id=? AND owner=?').bind(id,user).first();if(!row||(!row.file_key&&!(row.drive_file_id&&row.file_name)))throw new ApiError('File not found.',404);
